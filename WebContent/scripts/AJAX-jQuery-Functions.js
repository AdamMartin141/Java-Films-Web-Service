...
$(function() {
	...
	$("#getFilm-text-button").click(getFilmTEXT); 
	$("#getFilm-json-button").click(getFilmJSON); 
	$("#getFilm-xml-button").click(getFilmXML); 
		
	
	// getFilm by 'The Shawshank Redemption' title functions and their assigned HTML buttons
	$("#getShawshank-text-button").click(getShawshankTEXT);
	$("#getShawshank-json-button").click(getShawshankJSON);
	$("#getShawshank-xml-button").click(getShawshankXML);
		
	
	// deleteFilm, insertFilm and updateFilm functions and their assigned HTML buttons
	$("#insertFilm-button").click(insertFilmAJAX);
	$("#delete-film-button").click(deleteFilmAJAX);	
	$("#updateFilm-button").click(updateFilmAJAX);
...
//Function that retrieves 'The Shawshank Redemption' object from the database in text format, and posts it to index.html in a readable format
function getShawshankTEXT() {
	...
	$.ajax({
		url: "getFilm",
		type: "POST",
		data: "title=" + "The+Shawshank+Redemption" + "&format=" + "text",
		dataType: "text",
		...
		complete: function(responseBody) {
			...
			if (numberOfFilmAttributes == 6) {
				...
				// Displays a message in the specified HTML element to inform the user
				$("#get-shawshank-text-msg").html("Film record, 'The Shawshank Redemption' has been found" + ":");

				// Empties any previous results from the results div
				$('#get-shawshank-text-div').empty();
				...
				$('#get-shawshank-text-div').html(filmsTable);
				
				// jQuery UI effect - film table div slides in from the left 
				$('#get-shawshank-text-div').effect("slide", "right");
				
				// Confirms successful browser data formatting in the console, for debugging
				console.log("'The Shawshank Redemption' Text Film record successfully sent to the browser in a readable format");
			...
			if (numberOfFilmAttributes == 0) {
				...
				// Print an error message to the HTML page, and empty any previous results from the results div
				$("#get-shawshank-text-msg").html("Sorry, 'The Shawshank Redemption' has not been found");
				$('#get-shawshank-text-div').empty();
...
//Function that retrieves 'The Shawshank Redemption' film object from the database in JSON format, and posts it to index.html in a readable format
function getShawshankJSON() {
	...
	$.ajax({
		url: "getFilm",
		type: "POST",
		data: "title=" + "The+Shawshank+Redemption" + "&format=" + "json",
		dataType: "json",
		...
		if (numberOfFilms == 1) {
			...
			// Displays a message in the specified HTML element to inform the user
			$("#get-shawshank-json-msg").html("Film record, 'The Shawshank Redemption' has been found" + ":");

			// Empties any previous results from the results div
			$('#get-shawshank-json-div').empty();
			...
			// Places the results table in the HTML div
			$('#get-shawshank-json-div').html(filmsTable);
			
			// jQuery UI effect - film table div slides in from the left 
			$('#get-shawshank-json-div').effect("slide", "right");
			
			// Confirms successful browser data formatting in the console, for debugging
			console.log("'The Shawshank Redemption' JSON Film record successfully sent to the browser in a readable format");
		...
		if (numberOfFilms == 0) {
			...
			// Print an error message to the HTML page, and empty any previous results from the results div
			$("#get-shawshank-json-msg").html("Sorry, 'The Shawshank Redemption' has not been found");
			$('#get-shawshank-json-div').empty();
...
//Function that retrieves 'The Shawshank Redemption' film object from the database in XML format, and posts it to index.html in a readable format		
function getShawshankXML() {
	...
	$.ajax({
		url: "getFilm",
		type: "POST",
		data: "title=" + "The+Shawshank+Redemption" + "&format=" + "xml",
		dataType: "xml",
		...
		// Displays a message in the specified HTML element to inform the user
		$("#get-shawshank-xml-msg").html("Film record, 'The Shawshank Redemption' has been found");

		// Empties any previous results from the results div
		$('#get-shawshank-xml-div').empty();
		...
		// Places the results table in the HTML div
		$('#get-shawshank-xml-div').html(filmsTable);
			
		// jQuery UI effect - film table div slides in from the left 
		$('#get-shawshank-xml-div').effect("slide", "right");
					
		// Confirms successful browser data formatting in the console, for debugging
		console.log("'The Shawshank Redemption' XML Film record successfully sent to the browser in a readable format");
	...
	if (numberOfFilms == 0) {
		...
		// Print an error message to the HTML page, and empty any previous results from the results div
		$("#get-shawshank-xml-msg").html("Sorry, 'The Shawshank Redemption' has not been found");
		$('#get-shawshank-xml-div').empty();
...
//Function that inserts a film record and displays confirmation in the browser, without it refreshing or redirecting
function insertFilmAJAX() {
	...
	if (filmID && title && year && director && stars && review != null) {
		...
		$.ajax({
			...
			data: "id=" + filmID + "&title=" + title + "&year=" + year + "&director=" + director + 
...
//Function that updates a film record and displays confirmation in the browser, without it refreshing or redirecting
function updateFilmAJAX() {
	
	// Gets the typed film attributes from the HTML input box, and places them in JS variables
	filmID = $("#updateIDText").val();
	title = $("#updateTitleText").val();
	year = $("#updateYearText").val();
	director = $("#updateDirectorText").val();
	stars = $("#updateStarsText").val();
	review = $("#updateReviewText").val();

	
	// Updates the film if none of the data entry fields are empty/ 'null'
	if (filmID && title && year && director && stars && review != null) {
	...
	$("#update-film-text-msg").html("The Following Film Record has been Updated in the Database: " +
			"<ul>" +
			"<li>" + "ID:" + " " + filmID + "</li>" + 
			"<li>" + "Title:" + " " + title + "</li>" + 
			"<li>" + "Year:" + " " + year + "</li>" + 
	...
	// jQuery UI effect - confirmation message slides in from the left 
	$('#update-film-text-msg').effect("slide", "right");		
	
	console.log("Film record successfully updated")
	...
	$("#update-film-text-msg").html("Sorry, film not updated. Ensure that all fields are filled in.");
